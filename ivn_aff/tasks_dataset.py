...
class ObstaclesNavTask(Task[IThorEnvironment]):
    ...
    def _step(self, action) -> RLStepResult:
        ...
        # features = observation['maps']['feature']

        # 随机采集点
        locations = self.env.currently_reachable_points
        data_feature = []
        data_label = []
        while self.obs_count < 1000:
            pos = random.choice(locations)
            # del locations[locations.index(pos)]
            pos = [pos['x'], pos['z']]
            self.env.controller.step(
                action="Teleport",
                position=pos,
                rotation=dict(x=0, y=random.choice([0.0,90.0,180.0,270.0]), z=0),
            )
            if self.env.last_action_success:
                observation = self.get_observations(inter_mode=0)
                rgb = self.env.current_frame.copy()
                seg_dict = self.env.seg_ids
                for n,seg in enumerate(seg_dict):
                    if seg != 'none':
                        if seg not in self.obs_dicts.keys():
                            self.obs_dicts.update({seg: []})
                        self.obs_dicts[seg].append(observation['maps']['feature'][n,:])
                        self.obs_count += 1
            else:
                continue
        ...
        spawned = False
        spawn_objects = [o for o in self.env.all_objects() if o['objectId'] in self.obs_dicts.keys()]
        ...
        for spawn_object in spawn_objects:
            ...
            interactable_positions = self.env._interactable_positions_cache.get(
                scene_name=self.task_info['scene'], obj=spawn_object, controller=self.env.controller,
            )
            # 保留xyz坐标
            ...
            dis_to_target = [self.env.position_dist(dict(x=pos['x'], z=pos['z'], y=0), obj_pos, ignore_y=True) for
                                pos in
                                interactable_positions]
    
            if len(dis_to_target) > 0:
                position = interactable_positions[dis_to_target.index(random.choice(dis_to_target))]
        ...
        if visible_moveable_map[int(planner_pose_inputs[0]),int(planner_pose_inputs[1])]:
            ...
            # 判断推动方向
            # test_pos = [
            #     [moveable_centers[0][0]*2 - planner_pose_inputs[0], moveable_centers[0][1]*2 - planner_pose_inputs[1]],
            #     [moveable_centers[0][0] - (planner_pose_inputs[1]-moveable_centers[0][1]), moveable_centers[0][1] + (planner_pose_inputs[0]-moveable_centers[0][0])],
            #     [moveable_centers[0][0] + (planner_pose_inputs[1]-moveable_centers[0][1]), moveable_centers[0][1] - (planner_pose_inputs[0]-moveable_centers[0][0])]
            # ]
            # test_pos = [list(map(int, i)) for i in test_pos]
        ...
        # print(self.inter_index, l_step, np.where(target_map == 1), self.global_goal, np.where(self.goal_map == 1), planner_pose_inputs[:2], self.dist_to_target_(), self._rewards[-1], self.inter_mode)
        self.start = False
