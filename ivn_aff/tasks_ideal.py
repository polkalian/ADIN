...
class ObstaclesNavTask(Task[IThorEnvironment]):
    _actions = (MOVE_AHEAD, ROTATE_LEFT, ROTATE_RIGHT, LOOK_DOWN, LOOK_UP,
                ...
                END)
    # _actions = (MOVE_AHEAD, ROTATE_LEFT, ROTATE_RIGHT, LOOK_DOWN, LOOK_UP,
    #             MOVE_AHEAD, ROTATE_LEFT, ROTATE_RIGHT, LOOK_DOWN, LOOK_UP,
    #             MOVE_AHEAD, ROTATE_LEFT, ROTATE_RIGHT, LOOK_DOWN, LOOK_UP,
    #             DIRECTIONAL_AHEAD_PUSH, PICK_UP, DROP,
    #             END)
    ...
    def _plan(self, planner_inputs):
        ...
        # if not planner_inputs['goal_reachable']:
        #     goal_co = np.hstack(np.where(goal == 1))
        #     reachable_co = np.where((planner_inputs['exp_pred'].astype('bool') & traversible.astype('bool')) == 1)
        #     dist = np.array([np.sum(np.square(np.array([reachable_co[0][i],reachable_co[1][i]])-goal_co)) for i in range(reachable_co[0].shape[0])])
        #     min_10 = dist.argsort()[:10][::1]
        #     for i in min_10:
        #         goal[reachable_co[0][i], reachable_co[1][i]] = 1
    ...
    def act(self, action_str):
        ...
        if action_str == END:
            ...
            if self.mode == 'nav':
                self.mode = 'pick'
            self.pick_moves += 1
            obj = self.env.last_event.metadata['inventoryObjects'][0]
    ...
    def _step(self, action) -> RLStepResult:
        ...
        # action = action["lower"]
        # l_step = self.episode_length % num_local_steps
        self.inter_mode = 0

        # action = cast(int, action)
        # cpu_action = nn.Sigmoid()(torch.tensor(action)).numpy()
        cpu_action = action
        # print(cpu_action)

        # 生成long-term goal对应的map
        self.global_goal = [int(cpu_action[0] * self.map_shape[0]), int(cpu_action[1] * self.map_shape[1])]
        ...
        visible_moveable_map = skimage.morphology.binary_dilation(visible_moveable_map, skimage.morphology.disk(15))
        visible_pickable_map = skimage.morphology.binary_dilation(visible_pickable_map, skimage.morphology.disk(15))
        ...
        if visible_pickable_map[int(planner_pose_inputs[0]),int(planner_pose_inputs[1])]:
            ...
            # 计算物体中心视角
            # map mask障碍物
            # reachable_map_ = reachable_map | labelled_pickable
            # labelled_map_ = skimage.measure.label(reachable_map_, connectivity=1)
            # found_target_ = labelled_map_[np.where(self.goal_map_ == 1)[0][0],np.where(self.goal_map_ == 1)[1][0]] == labelled_map_[int(planner_pose_inputs[0]),int(planner_pose_inputs[1])] != 0
            # if not found_target and found_target_:
            # if d_angle < 45.0:
            if True:
                ...
                if self.pick_index <= 5:
                    ...
                    self.anti_pick |= skimage.morphology.binary_erosion(labelled_pickable, skimage.morphology.disk(15))
        ...
        if visible_moveable_map[int(planner_pose_inputs[0]),int(planner_pose_inputs[1])]:
            ...
            d_angle = np.abs(angle - planner_pose_inputs[2]) if np.abs(angle - planner_pose_inputs[2]) <= 180.0 else 360.0 - np.abs(angle - planner_pose_inputs[2])
            # map mask障碍物
            # reachable_map_ = reachable_map | labelled_moveable
            # labelled_map_ = skimage.measure.label(reachable_map_, connectivity=1)
            # found_target_ = labelled_map_[np.where(self.goal_map_ == 1)[0][0],np.where(self.goal_map_ == 1)[1][0]] == labelled_map_[int(planner_pose_inputs[0]),int(planner_pose_inputs[1])] != 0
            # if not found_target and found_target_:
            # if d_angle < 45.0:
            if True:
                ...
                if self.pick_index <= 5:
                    ...
                    self.anti_move |= skimage.morphology.binary_erosion(labelled_moveable, skimage.morphology.disk(15))
                self.tmp = planner_pose_inputs[:2]
        
        # rgb = observation['rgb']
        
        # if self.goal_reached and self.goal_location in [1,2]:
        #     self.inter_mode = self.goal_location
        #     self.pick_index = 0
        self.goal_reached = False
        k = True
        # self.goal_reward = 0
        ...
        for l_step in range(num_local_steps):
            ...
            # reachable_map = skimage.morphology.binary_erosion(reachable_map, self.selem)
            # labelled_map = skimage.measure.label(reachable_map, connectivity=1)
            self.found_target = int(True in (explored_map & target_map))
            ...
            # if labelled_map[int(planner_pose_inputs[0]),int(planner_pose_inputs[1])] == 0:
            #     print("erosion too much")
            # dist = np.sum(np.square(np.array([np.where(target_map == 1)[0][0], np.where(target_map == 1)[1][0]])-np.array(self.global_goal)))
            ...
            # if False:
                ...
                if self.inter_mode == 1:
                    action_str = random.choice([DIRECTIONAL_AHEAD_PUSH, DIRECTIONAL_BACK_PUSH, DIRECTIONAL_RIGHT_PUSH, DIRECTIONAL_LEFT_PUSH])
                elif self.inter_mode == 2:
                    pick_sequence = [PICK_UP, ROTATE_RIGHT, ROTATE_RIGHT, DROP, ROTATE_RIGHT, ROTATE_RIGHT]
                    action_str = PICK_UP
                
                # action_id = self._actions.index(action_str)
                # if self.goal_location == 0 and (self.goal_location == 1 or self.pick_index == len(pick_sequence)):
                #     self.inter_mode = 0
                #     break
            else:
                # 进入导航阶段
                ...
                # p_input['map_pred'] = semantic_map[:,:,0].T # (np.ones((self.map_shape[0], self.map_shape[1]))-semantic_map[:,:,0]).T
                ...
                # 如果长时间卡住怎么办
                if len(self.path) > 2:
                    # if len(set([(p['x'],p['z']) for p in self.path[-20:]])) <= 2 and len(self.path) >= 20:  and not self.env.last_action_success
                    #     self.count += 1
                ...
                if self.count >= 5:
                    # if self.inter_mode == 1:
                    #     action_str = DIRECTIONAL_AHEAD_PUSH
                    # elif self.inter_mode == 2:
                    #     action_str = PICK_UP
                    # else:
                ...
                # else:
                #     self.inter_mode = 0
                # if self.count == 30 or (len(set([(p['x'],p['z']) for p in self.path[-30:]])) <= 2 and len(self.path) >= 50):
                #     action_str = END
            ...
            # print(self.action_str, np.where(target_map == 1), self.global_goal, np.where(self.goal_map == 1), planner_pose_inputs[:2], self.dist_to_target_())

            #执行动作
            self.act(action_str)

            if action_str in [END,DIRECTIONAL_AHEAD_PUSH, DIRECTIONAL_BACK_PUSH, DIRECTIONAL_RIGHT_PUSH, DIRECTIONAL_LEFT_PUSH,PICK_UP]:
                break
        ...
        print(self.pick_index, l_step, np.where(target_map == 1), self.global_goal, np.where(self.goal_map == 1), planner_pose_inputs[:2], self.dist_to_target_(), self._rewards[-1], self.inter_mode)
        self.start = False
    ...
    def shaping(self) -> float:
        ...
        # if self.last_geodesic_distance > 0 and geodesic_distance == -1.0:
        #     rew = -1.0
    ...
    def shaping_by_path(self) -> float:
        ...
        if self.action_str in [MOVE_AHEAD, ROTATE_LEFT, ROTATE_RIGHT, END]:
            ...
            if (
                ...
                # print(self.last_geodesic_distance_, geodesic_distance_)
                reward += self.last_geodesic_distance_ - geodesic_distance_
                self.nav_reward += reward
                # self.distance_change += geodesic_distance - self.last_geodesic_distance
                # if self.last_geodesic_distance - geodesic_distance > 0:
                #     # self.effective_moves += 1
                #     reward += 0.1
                #     print(self.last_geodesic_distance, geodesic_distance)
                # print(self.last_action_success, reward)
        self.last_geodesic_distance_ = geodesic_distance_
        return reward

    def shaping_by_action(self) -> float:
        reward = 0.0
        # if self.action_str in [DIRECTIONAL_AHEAD_PUSH, DIRECTIONAL_BACK_PUSH,
        #                             DIRECTIONAL_RIGHT_PUSH, DIRECTIONAL_LEFT_PUSH,
        #                             PICK_UP, DROP]:
        #     if self.last_action_success:
        #         reward += 0.1
        ...
        # if self.effective_moves == 1:
        #     reward -= 0.03

        # if self.env.last_action == MOVE_AHEAD:
        #     self.nav_moves += 1
        #     if len(self.path) > 1 and self.path[-1] != self.path[-2]:
        #         reward += 0
        #         self.nav_success += 1
        #     else:
        #         reward -= 0
                # print(self.env.last_event.metadata["errorMessage"])
        return reward
    ...
    def judge(self) -> float:
        ...
        # print(reward)
        # reward += self.shaping_by_action()
        reward += self.shaping()  #交互
        # print(self.nav_reward, self.inter_reward)

        # print(self.mode, self.action_str, self.push_val, self.pick_val)
        # if self.mode == 'nav' and self.action_str in [DIRECTIONAL_AHEAD_PUSH, DIRECTIONAL_BACK_PUSH, DIRECTIONAL_RIGHT_PUSH, DIRECTIONAL_LEFT_PUSH]:
        #     if self.push_val:
        #         reward += 0.1
        #     else:
        #         reward -= 0
        # if self.mode == 'nav' and self.action_str in [PICK_UP, DROP]:
        #     if self.pick_val:
        #         reward += 0.1
        #     else:
        #         reward -= 0
        ...
        if self._took_end_action:
            ...
            # reward += self.reward_configs["goal_success_reward"] * 0.5
