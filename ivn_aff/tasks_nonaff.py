...
class ObstaclesNavTask(Task[IThorEnvironment]):
    ...
    def _step(self, action) -> RLStepResult:
        ...
        for l_step in range(num_local_steps):
            ...
            if l_step > 0:
                observation = self.get_observations(inter_mode=self.inter_mode if self.count >= 10 and self.inter_index <= 5 else 0)
        ...
        # 如果推动，转一圈刷新地图
        if self.action_str == 'DirectionalPush':
            for i in range(4):
